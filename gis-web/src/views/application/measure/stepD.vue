...
<template>
  <div>
    <div class="title">暴雨洪涝灾害下的城市韧性测度水平</div>
    <div class="layout-main">
      ...
      <div class="panel-middle">
        ...
        <div class="layer-switch-container">
          ...
          <span style="margin-right: 100px;">暴雨洪涝灾害</span>
        </div>
        <!-- 时间 -->
        <div class="time-step-controler">
          ...
          <el-button style="margin-left: 16px" @click="next"
            :disabled="timeControlDisabled || activeTimeIndex > maxTimeIndex - 1" type="primary">下一时刻
          </el-button>
        </div>
        <!-- 基础图层开关 -->
        <div class="layer-switch-container">
...
export default {
  ...
  data() {
    return {
      ...
      type: 'flood',
      currentSimulationType: 'flood',          // 当前模拟任务类型
      infoTask: {                         //当前任务信息
        computeTime: 0
      },
  ...
  methods: {
    ...
    async init() {
      ...
      // 获取模拟任务结果
      requestUtils.getTaskResult(this, this.task, () => {
        this.timeControlDisabled = false
        mapUtils.buildRiskLayer(this, this.task)
      })
