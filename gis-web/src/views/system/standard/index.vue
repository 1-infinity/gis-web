...
<!--        </div>-->
        ...
        <div  id="menu" v-show="optionCardShow" @mouseleave="optionCardShow=!optionCardShow">
          <el-card class="box-card">
            <div class="text item">
              <el-link icon="el-icon-circle-plus-outline" @click="() => treeAdd()" :underline="false">添加分类</el-link>
            </div>
            <div class="text item">
              <el-link icon="el-icon-edit" @click="() => treeUpdate()" :underline="false">修改分类</el-link>
            </div>
            <div class="text item">
              <el-link icon="el-icon-remove-outline" @click="() => treeRemove()" :underline="false">删除分类</el-link>
            </div>
          </el-card>
        </div>
      </el-col>
...
<!--        <el-card>-->
        <div>
        <el-form style="width: 100%;float: left;margin-top: 0px;margin-left: 10px" :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="70px">
        ...
        <el-row style="float: left;margin-top: 0px;margin-left: 5px;margin-bottom: 20px" :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              type="primary"
              plain
...
<!--        </el-card>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px"
               ...
               :show-close="false"
               :close-on-click-modal="false" class="standardInfo"
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="110px" style="padding-right: 20px;">
        <el-row>
          <el-col :span="12">
            <el-form-item label="标准中文名称" prop="chineseName">
              <el-input v-model="form.chineseName" placeholder="标准中文名称" :disabled="infoType == 'info'" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<!--        </el-row>-->
        <el-row>
          <el-col :span="12">
            <el-form-item label="标准号" prop="stdNo">
              <el-input v-model="form.stdNo" placeholder="标准号" :disabled="infoType == 'info'" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="关联网址" prop="relatedUrl">
              <el-input v-model="form.relatedUrl" placeholder="关联网址" :disabled="infoType == 'info'" />
            </el-form-item>
          </el-col>
<!--          <el-col :span="12">-->
...
<!--        </el-row>-->
        <el-form-item label="附件" prop="attachment">
          <upload-file v-if="infoType == 'edit' || infoType == 'add'" @input="fileUploadResult" @remove="fileRemove" :file-size="50" :fileType="['pdf','doc','docx','jpg']" :value="attachment"></upload-file>
          <el-table v-if="infoType == 'info'" :data="attachment" :show-header="false">
            <el-table-column >
              <template slot-scope="scope">
        ...
        <el-form-item label="备注" prop="remark">
          <el-input v-model="form.remark" type="textarea" :disabled="infoType == 'info'"></el-input>
        </el-form-item>
      </el-form>
    ...
    <!-- 添加或修改参数配置对话框 -->
    <el-dialog :title="title" :visible.sync="treeOpen" width="800px"
               :destroyOnClose='destroyOnClose'
               :show-close="false"
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" v-if="infoType == 'add' || infoType == 'edit'" @click="submitTreeForm">确 定</el-button>
        <el-button @click="treeCancel">取 消</el-button>
      </div>
    </el-dialog>
    <!-- 文档下载对话框   -->
    <el-dialog :title="downloadTitle" :visible.sync="openDownload" width="500px" :destroyOnClose='destroyOnClose'
...
<script>
import { listData, getData,addStandardData,delStandardData, updateStandardData   } from "@/api/system/standard";
import { TreeData,addData,delData, updateData,getTreeData } from "@/api/system/standardCatalog";
      ...
      listData(this.queryParams).then(response => {
        if (response.rows && response.rows.length > 0) {
          ...
          this.standardList.forEach((item) => {
            item.files = item.attachment != null ? JSON.parse(item.attachment) : null;
            item.url = item.files != null && item.files.length > 0 ? this.baseUrl+item.files[0].url: null;
            item.file = item.files != null && item.files.length > 0 ? item.files[0]: null;
          });
        }
