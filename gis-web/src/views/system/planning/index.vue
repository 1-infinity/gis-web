...
<!--          </el-col>-->
          <el-col :span="24">
            <el-form-item label="描述">
              <el-input v-model="form.discription" type="textarea" :min-height="192"></el-input>
<!--              <editor v-model="form.discription" :min-height="192"/>-->
            </el-form-item>
          </el-col>
          <el-col :span="16">
            <el-form-item label="关联标准" prop="standardId">
              <input v-model="form.standardId" type="hidden"/>
<!--              :style="{width:infoType == 'info'?'100%':'70%'}"-->
              <el-input  v-model="form.associateStandardName" style="width:70%;" readonly>
<!--                infoType != 'info' &&-->
                <el-button slot="append" icon="el-icon-delete" v-if=" form.associateStandardName != null"
                           @click="clearSelectStandard()"></el-button>
              </el-input>
<!--              v-if="infoType != 'info'"-->
              <el-button style="width: 27%;margin-left: 2%;padding: 10px" type="primary" @click="selectStandard()">选择标准
              </el-button>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="截图" prop="attachment">
              <upload-file  @input="fileUploadResult" @remove="fileRemove" :fileType="['JPG','PNG','png','jpg']" :limit="1" :value="attachment"></upload-file>
...
<!--    <el-dialog :title="title" :visible.sync="selectOpen"  width="900px"  style="height: 1000px;scroll-behavior: auto">-->
    <el-dialog :title="title" :visible.sync="selectOpen"  :fullscreen="true">
      <standard-select @select="selectChange" style="margin-top: -30px"></standard-select>
      <div slot="footer" class="dialog-footer" style="margin-top: -40px;margin-bottom: -20px;margin-right: 50px ">
        <el-button type="primary" @click="submitRelate">关联标准</el-button>
        <el-button @click="cancelRelate">取 消</el-button>
      </div>
    </el-dialog>

    <!--  标准弹窗  -->
    <el-dialog :close-on-click-modal = "false" :title="standardSelectDialog.title" :visible.sync="standardSelectDialog.open" width="800px" append-to-body>
      <el-form :model="standardQueryParams" ref="standardQueryParams" :inline="true" label-width="70px">
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" size="mini" @click="handleStandardQuery(false)">搜索</el-button>
          <el-button icon="el-icon-refresh" size="mini" @click="resetStandardQuery">重置</el-button>
        </el-form-item>
      </el-form>
      <el-table :data="standardList" ref="standardTable" @select-all="cancelAllStandardSelection"
                @select="handleStandardSelection">
        <el-table-column type="selection" width="55" align="center"/>
        <el-table-column label="标准名称" align="left" prop="chineseName" />
        <el-table-column label="标准号" align="left" prop="stdNo" :show-overflow-tooltip="true" />
      ...
      />
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="confirmSelectStandard">确 定</el-button>
...
<script>
import { listPlanning, getPlanning, delPlanning, addPlanning, updatePlanning } from "@/api/system/planning";
  ...
  methods: {
    ...
    getList() {
      ...
      listPlanning(this.queryParams).then(response => {
        ...
        this.planningList.forEach((item) => {
          item.attachment = item.attachment != null ? JSON.parse(item.attachment) : null;
          item.url = item.attachment != null && item.attachment.length > 0 ? this.baseUrl+item.attachment[0].url: null;
        });
    ...
    confirmSelectStandard() {
      if (this.selectedStandard) {
        ...
        this.form.associateStandardName = this.selectedStandard.chineseName + '(' + this.selectedStandard.stdNo + ')'
      } else {
        this.form.standardId = null
        this.form.associateStandardName = null
      }
